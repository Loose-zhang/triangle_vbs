...
载波相位（周）：两站该频点**载波相位均有效**（非零，与 RTKLIB 一致）时取算术平均：

```text
L = (L_1 + L_2) / 2
```

若任一站在该观测码上没有有效载波，则合成输出该频点上 **`L` 为 0**（不写伪距推导的占位载波）。

该方法适合演示和短基线近似，不等价于完整的物理观测重构。两站整周模糊度不同，简单平均在物理上不严密；实际高精度应用仍需处理接收机钟差、电离层、相位模糊度等。大范围或长基线时请谨慎使用虚拟中点载波。
...
注意：只有两站均有有效载波时，`L` 才为**两站算术平均**；否则该频点 **`L`=0**。码重映射生成 MSM 时也不会用伪距从 0「补」成载波。
