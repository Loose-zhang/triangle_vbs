...
/* 添加extern声明以访问主程序中的strpath */
extern char strpath[8][MAXSTA];
...
extern int rtkoutstat(rtk_t *rtk, int level, char *buff)
{
    ...
    if (rtk->opt.mode>=PMODE_PPP_KINEMA) {
        ...
        /* 查找strpath的最后一个非空元素作为挂载点
        int mount_idx = 0;
        for (int i = 0; i < 8; i++) {
            if (strpath[i][0] != '\0') mount_idx = i;
        }
        p+=pppoutstat(rtk,buff,strpath[mount_idx]);*/
	char mountpoint[256] = "";
        // 只对类似ntrip路径（有@且有/）做挂载点提取
	if (strchr(strpath[0], '@') && strrchr(strpath[0], '/')) {
		const char *p = strrchr(strpath[0], '/');
		if (p && *(p+1)) {
			strncpy(mountpoint, p+1, sizeof(mountpoint)-1);
			mountpoint[sizeof(mountpoint)-1] = '\0';
		}
	}
	p += pppoutstat(rtk, buff, mountpoint);
