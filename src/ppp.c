...
/* 提取NTRIP挂载点 -------------------------------------------------
void extract_mountpoint(const char *strpath, char *mountpoint, int maxlen) {
    const char *p = strchr(strpath, '/');
    if (p) {
        strncpy(mountpoint, p + 1, maxlen - 1);
        mountpoint[maxlen - 1] = '\0';
        // 去除后续参数
        char *q = strchr(mountpoint, ':');
        if (q) *q = '\0';
    } else {
        mountpoint[0] = '\0';
    }
...
/* write solution status for PPP ---------------------------------------------*/
int pppoutstat(rtk_t *rtk, char *buff,const char *mountpoint)
{
    ssat_t *ssat;
    ...
    /* receiver position */
    /*p += sprintf(p, "$POS,%s,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%s\n",
                 sol_time_str, rtk->sol.stat, x[0], x[1], x[2],
                 STD(rtk, 0), STD(rtk, 1), STD(rtk, 2), mountpoint);*/

    ecef2pos(x,pos);
    p += sprintf(p, "$POS,%s,%d,%.8f,%.8f,%.4f,%.4f,%.4f,%.4f,%s\n",
                 sol_time_str, rtk->sol.stat, pos[0]*R2D, pos[1]*R2D, pos[2],
                 STD(rtk, 0), STD(rtk, 1), STD(rtk, 2), mountpoint);
    ...
    /* receiver clocks */
    i = IC(0, &rtk->opt);
    p += sprintf(p, "$CLK,%s,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%s\n",
                 ...
                 STD(rtk, i + 3) * 1E9 / CLIGHT,mountpoint);

    /* tropospheric parameters */
    if (rtk->opt.tropopt == TROPOPT_EST || rtk->opt.tropopt == TROPOPT_ESTG) {
        i = IT(&rtk->opt);
        p += sprintf(p, "$TROP,%s,%d,%d,%.4f,%.4f,%s\n",
                     sol_time_str, rtk->sol.stat, 1, x[i], STD(rtk, i),mountpoint);
    }
    if (rtk->opt.tropopt == TROPOPT_ESTG) {
        i = IT(&rtk->opt);
        p += sprintf(p, "$TRPG,%s,%d,%d,%.5f,%.5f,%.5f,%.5f,%s\n",
                     sol_time_str, rtk->sol.stat, 1, x[i + 1], x[i + 2],
                     STD(rtk, i + 1), STD(rtk, i + 2),mountpoint);
    }
    ...
    for (i = 0; i < MAXSAT; i++) for (j = 0; j < NF(&rtk->opt); j++) {
        ...
        satno2id(i + 1, id);
        p += sprintf(p, "$AMB,%s,%d,%s,%d,%.4f,%.4f,%s\n",
                     sol_time_str, rtk->sol.stat, id, j + 1, x[k], STD(rtk, k),mountpoint);
    }
